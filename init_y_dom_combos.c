...
int init_y_dom_combos(struct state_struct *state, 
		      double **ddata_p, 
		      int **imeta_data_p,
		      int *yfd_p,
		      int *tsv_offset_p) {
  /*
    Initialize the *.y.dat file and meta data.
    Called by: gen_y_dom_combos
...
4     Total bytes per index, tbpi (8) (#3 * #1)/8 rounded up to a multiple of 8.
      tbpi  = ((sbpi/8) * nc) + 7) & 248

5     Bytes per ts value         (8) bptsv

6     Total bytes per entry. bpe = 4*bptsv + tbpi (>= 40)
      This changed to 4 doubles as we store the 3 tristimulus values
      and then a v_b value for green channels or a u_b value for
      red channels. We sort on the v_b/u_b field.
...
16:19 Number of extra records to store mapping,ner: = 0 if 3bpe-96 <= nc
      else ner = ceil (nc/bpe)

20:23 Sixth 4 byte integer, 
      Number of bins for sorted tristimulus,vb,ub records, num_bins      
...
56:63 8byte double combo_bin_width.
...
*/
  ...
  yfp = fopen(y_dom_file,"w+");
  if (yfp == NULL) {
    success = 0;
    if (lfp) {
      fprintf(lfp,"init_y_dom_combos: Error, could not open %s\n",
	      y_dom_file);
      fflush(lfp);
    }
  }
  if (success) {
    ...
    */
    ...
    /*
      ...
      + 2* bptsv for ub_value and vb_value.
    */
    bpe   = (5 * bptsv) + tbpi;
    /*
    ...
    */
    ...
    /*
      ...
      memory map the file.
    */
    yfd_size  = 2*(max_elems * bpe) + md_len;
    y_dom_vb_max = loop_state->y_dom_vb_max;
    y_dom_vb_min = loop_state->y_dom_vb_min;
    /*
      This value of num_bins fails when y_dom_vb_max is very large as it
      may be for some of the smaller problems from Michael Royer.
    num_bins  = (int)(((y_dom_vb_max - y_dom_vb_min)/combo_bin_width)+ 1.5);
    ...
    */
    ...
    if ((num_bins *8) > max_elems * bpe) {
      yfd_size = md_len + (max_elems*bpe) + (num_bins * 8);
    }
    ierr      = ftruncate(yfd,yfd_size);
    save_err  = errno;
    if (ierr < 0) {
      success = 0;
      if (lfp) {
	fprintf(lfp,
		"init_y_dom_combos: Error unable to truncate"
		" %s to %ld bytes, err = %d, %s\n",
		y_dom_file,yfd_size,save_err,strerror(save_err));
	fflush(lfp);
      }
    }
  }
  if (success) {
    ...
    flags     = MAP_SHARED;
    md_addr   = mmap(null_addr,yfd_size,prot,flags,yfd,offset);
    save_err  = errno;
    if (md_addr == MAP_FAILED) {
      success = 0;
      if (lfp) {
	fprintf(lfp,
		"init_y_dom_combos: Error unable to mmap %s, error = %d, %s\n",
  ...
  if (success) {
    ...
    if (mdl > 255) {
      success = 0;
      if (lfp) {
	fprintf(lfp,"init_y_dom_combos: Error, meta data length = %d > 255\n",
		mdl);
      }
    }
  }
  if (success) {
    ...
    *dmd_p = min_fract;
    /*
      ...
      NB this will be recomputed in gen_y_dom_combos.
    */
    combo_bin_width = (y_dom_vb_max - y_dom_vb_min)/(num_bins -1);
    ...
    *dmd_p = combo_bin_width;
    ...
    pack_imap(ny,&imap_sort[nz],cimap);
