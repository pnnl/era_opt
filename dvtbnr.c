...
#include "mpi.h"
#include "dvtbnr.h"
int dvtbnr(int n, double *dv, double *left_buffer, double *right_buffer, 
	   int itag, struct comm_map_struct *reduction_map, FILE *lfp) {
  /*
    Double Vector Tree-Based Minimum Reduction
  ...
  */
  if (left_child >= 0) {
	...
	fprintf(lfp,"dvtbnr: Error return from MPI_Irecv from %d was %d, %s\n",left_child,
		ierr, mpi_string);
	fflush(lfp);
      }
    }
  ...
  */
  ...
  if (success) {
	...
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"dvtbnr: Error return from MPI_Wait from %d was %d, %s\n",left_child,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      } else {
  ...
  if (success) {
	...
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"dvtbnr: Error return from MPI_Send to %d was %d,%s\n",parent,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      }
