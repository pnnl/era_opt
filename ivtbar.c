...
#include "mpi.h"
#include "ivtbar.h"
int ivtbar(int n, int64_t *iv, int64_t *left_buffer, int64_t *right_buffer, 
	   int itag, struct comm_map_struct *reduction_map, FILE *lfp) {
  /*
    Integer Vector Tree-Based Additive Reduction
  ...
  */
  if (left_child >= 0) {
	...
	fprintf(lfp,"ivtbar: Error return from MPI_Irecv from %d was %d, %s\n",left_child,
		ierr, mpi_string);
	fflush(lfp);
      }
    }
  ...
  */
  ...
  if (success) {
	...
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"ivtbar: Error return from MPI_Wait from %d was %d, %s\n",left_child,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      } else {
  ...
  if (success) {
	...
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"ivtbar: Error return from MPI_Send to %d was %d,%s\n",parent,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      }
