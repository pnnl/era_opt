...
#include "mpi.h"
#include "dvtbxr.h"
int dvtbxr(int n, double *dv, double *left_buffer, double *right_buffer, 
	   int itag, struct comm_map_struct *reduction_map, FILE *lfp) {
  /*
    Double Vector Tree-Based Maximum Reduction
    for a vector of doubles of length n, using provided buffer space
    ...
    n                           i0i    length in elements of the dv vector.

    dv                          d1b    local vector of vectors to be reduced.
                                       On rank 0 on ouput it has the final reduction.
				       On other ranks it has intermediate partial sums,
				       and should not be used.

    left_buffer                 d1w    buffer for receiving vector from left child.
                                       length is n.

    right_buffer                d1w    buffer for receiving vector from right child.
  ...
  */
  if (left_child >= 0) {
	...
	fprintf(lfp,"dvtbxr: Error return from MPI_Irecv from %d was %d, %s\n",left_child,
		ierr, mpi_string);
	fflush(lfp);
      }
    }
  ...
  */
  ...
  if (success) {
	...
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"dvtbxr: Error return from MPI_Wait from %d was %d, %s\n",left_child,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      } else {
  ...
  if (success) {
    if (parent >= 0) {
      ierr = MPI_Send(dv,n,msg_type,parent,itag,comm);
      if (ierr != MPI_SUCCESS) {
	success = 0;
	if (lfp) {
	  MPI_Error_string(ierr,mpi_string,&err_len);
	  fprintf(lfp,"dvtbxr: Error return from MPI_Send to %d was %d,%s\n",parent,ierr,
		  mpi_string);
	  fflush(lfp);
	}
      }
